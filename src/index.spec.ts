...
function createGet(config?: { options?: TGetOptions, defaultOptions?: TDefaultAPIOptions }) {
	return () => createClient(config?.defaultOptions).get(config?.options);
}

function createPost(config: { options: TPostOptions, defaultOptions?: TDefaultAPIOptions }) {
	return () => createClient(config.defaultOptions).post(config.options);
}

function createPut(config: { options: TPutOptions, defaultOptions?: TDefaultAPIOptions }) {
	return () => createClient(config.defaultOptions).put(config.options);
}

function createDelete(config?: { options?: TDeleteOptions, defaultOptions?: TDefaultAPIOptions }) {
...
describe("Default", () => {
	...
	it.each([
		...
		expect(fetchSpy.mock.calls[0][0]).toBe("/foo?pokemon=pikachu&id=1&isPremium=false");
	});
	...
	])("Should call fetch with no query string when query object is empty", async (method) => {
		const fetchSpy = vi
			.spyOn(globalThis, "fetch")
			.mockImplementation(async () => new Response());
	...
	const headersExpectation = new Headers({ "content-type": "application/json" });
	it.each([
		createGet({
			defaultOptions: {
		...
		expect(fetchSpy).toHaveBeenCalledOnce();
		expect(fetchSpy.mock.calls[0][1]?.headers).toStrictEqual(headersExpectation);
	});
	...
	it.each([
		...
		expect(fetchSpy).toHaveBeenCalledOnce();
		expect(fetchSpy.mock.calls[0][1]?.credentials).toStrictEqual(credentialsExpectation);
	});
});
...
describe("Options", () => {
	...
	it.each([
		createPost(bodyExpectation),
		createPut(bodyExpectation),
	])("Should call fetch with correct body", async (method) => {
		const fetchSpy = vi
			.spyOn(globalThis, "fetch")
			.mockImplementation(async () => new Response());
		...
		expect(fetchSpy.mock.calls[0][1]?.body).toStrictEqual(JSON.stringify(bodyExpectation.options.body))
	...
	it.each([
		...
		vi
			.spyOn(globalThis, "fetch")
			.mockImplementation(async () => new Response(JSON.stringify(apiJson), {
				headers: {
